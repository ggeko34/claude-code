...
# Usage: curl -fsSL https://raw.githubusercontent.com/ggeko34/claude-code/main/install.sh | bash
...
REPO="https://github.com/ggeko34/claude-code.git"
...
ART
  printf "${RESET}"
  printf "${DIM}  The fucking build of Claude Code${RESET}\n"
  echo ""
}
...
build_binary() {
  info "Building claude-code (all experimental features enabled)..."
  cd "$INSTALL_DIR"
...
link_binary() {
  ...
  ok "Symlinked: $link_dir/claude-code"
...
printf "    ${CYAN}claude-code${RESET}                          # interactive REPL\n"
printf "    ${CYAN}claude-code -p \"your prompt\"${RESET}          # one-shot mode\n"
echo ""
printf "  ${BOLD}Set your API key:${RESET}\n"
printf "    ${CYAN}export ANTHROPIC_API_KEY=\"sk-ant-...\"${RESET}\n"
echo ""
printf "  ${BOLD}Or log in with Claude.ai:${RESET}\n"
printf "    ${CYAN}claude-code /login${RESET}\n"
echo ""
printf "  ${DIM}Source: $INSTALL_DIR${RESET}\n"
printf "  ${DIM}Binary: $INSTALL_DIR/cli-dev${RESET}\n"
printf "  ${DIM}Link:   ~/.local/bin/claude-code${RESET}\n"
echo ""
